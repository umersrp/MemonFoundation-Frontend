 ...
 * - Single long-scroll form (Section A, B, C)
 * - Modal with Important Guidelines & Terms (must 'I Agree')
 * - File uploads for all listed documents
 * - Posts multipart/form-data to http://localhost:5000/api/users/register
 * - Matches backend schema field names (academicRecords, extracurricularActivities, father, mother, financialInformation, officeUseInfo, familyMembers, etc.)
 *
 * NOTE: Ensure <ToastContainer /> is present in App root.
 */
...
const StudentRegistrationForm = () => {
  ...
  // Form data: fields aligned with your schema
  const [form, setForm] = useState({
    // Top-level user fields expected by schema
    ...
    // academicRecords (array of objects)
    academicRecords: [
      {
        class: "",
   ...
   officeUseInfo: {
    ...
    // Extra personal fields from form
  ...
  const setField = (path, value) => {
    // path can be "father.firstName" or "academicRecords[0].schoolName"
    if (!path.includes(".")) {
    ...
    setForm((prev) => {
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        const part = parts[i];
        // array index e.g. academicRecords[0]
  ...
  const removeFile = (key, i) => {
    setFiles((prev) => ({ ...prev, [key]: prev[key].filter((_, idx) => idx !== i) }));
  };
  const openFileDialog = (key) => fileInputRefs.current[key]?.click();
  ...
  const validate = () => {
    ...
    // at least one academic record must be filled
    ...
    // required files: birthCertificate & last3Results
  ...
  const handleSubmit = async (ev) => {
    ...
    // compose schema-compliant top-level 'name' from first/middle/last
    const composed = `${form.firstName} ${form.middleName || ""} ${form.lastName}`.trim();

    // create payload object to string-append
    const payloadObj = {
      type: "student",
      name: composed,
      email: form.email,
      ...
      father: {
        ...
        middleName: form.father.middleName,
        lastName: form.father.lastName,
        cnicNo: form.father.cnicNo,
        jamaatName: form.father.jamaatName,
        jamaatMembershipNo: form.father.jamaatMembershipNo,
        profession: form.father.profession,
        companyName: form.father.companyName,
        designation: form.father.designation,
        residentialAddress: form.father.residentialAddress,
        mobileNumber: form.father.mobileNumber,
        email: form.father.email,
      },

      mother: {
        firstName: form.mother.firstName,
        middleName: form.mother.middleName,
        lastName: form.mother.lastName,
        cnicNo: form.mother.cnicNo,
        jamaatName: form.mother.jamaatName,
        jamaatMembershipNo: form.mother.jamaatMembershipNo,
        profession: form.mother.profession,
        companyName: form.mother.companyName,
        designation: form.mother.designation,
        residentialAddress: form.mother.residentialAddress,
        mobileNumber: form.mother.mobileNumber,
        email: form.mother.email,
      },

      selectionNote: form.selectionNote || "",

      numberOfHouseholdMembers: Number(form.numberOfHouseholdMembers) || 0,
      familyMembers: form.familyMembers.map((fm) => ({ name: fm.name, relation: fm.relation })),
      ...
      officeUseInfo: {
        jamaatName: form.officeUseInfo.jamaatName,
        membershipNumber: form.officeUseInfo.membershipNumber,
        belongsToJamaat: form.officeUseInfo.belongsToJamaat,
        supportProvided: form.officeUseInfo.supportProvided,
        zakatDeserving: !!form.officeUseInfo.zakatDeserving,
        authorizedSignature: form.officeUseInfo.authorizedSignature,
        channelOfSubmission: form.officeUseInfo.channelOfSubmission,
        memfOffice: {
          studentCode: form.officeUseInfo.memfOffice.studentCode,
          assessmentDate: form.officeUseInfo.memfOffice.assessmentDate,
          interviewDate: form.officeUseInfo.memfOffice.interviewDate,
          decision: form.officeUseInfo.memfOffice.decision,
          category: form.officeUseInfo.memfOffice.category,
          scholarship: {
            grantedFor: form.officeUseInfo.memfOffice.scholarship.grantedFor || [],
            totalAmount: Number(form.officeUseInfo.memfOffice.scholarship.totalAmount) || 0,
          },
          panelComments: form.officeUseInfo.memfOffice.panelComments || "",
        },
      },

      // include additional fields for form display if needed
      personal: {
        gender: form.gender,
        dob: form.dob,
    ...
    try {
      ...
      navigate("/thank-you"); // adjust route if you have different page
    } catch (err) {
            ...
            <div
              ...
              <form onSubmit={handleSubmit} className="space-y-8 m-10">
                ...
                <Card title={"SECTION A: PERSONAL INFORMATION"} >
                  <div className="p-4">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                      ...
                      <select value={form.gender} onChange={(e) => setField("gender", e.target.value)} className="mt-1 w-full border rounded p-2 border-gray-300">
                      ...
                      {/* <div>
                        ...
                        <select value={form.examinationBoard} onChange={(e) => setField("examinationBoard", e.target.value)} className="mt-1 w-full border rounded p-2 border-gray-300">
                        ...
                        <div key={idx} className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end mb-2">
                          ...
                          <div>
                            ...
                            <Select
                              ...
                              onChange={(selected) =>
                                setField(`academicRecords[${idx}].stream`, selected?.value || "")
                              }
                ...
                {/* <section className="bg-white p-4 rounded-md shadow-sm">
                  <h4 className="font-semibold mb-3">Supporting Documents (uploads)</h4>
                  <p className="text-sm text-gray-600 mb-3">Upload clear scans/photos of the following documents. Multiple files allowed where applicable.</p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {requiredFilesList.map((f) => (
                      <div key={f.key} className="p-3 border rounded">
                        ...
                        <div className="flex gap-2 items-center">
                          <input
                            ...
                            className="hidden"
                          />
                          <button type="button" onClick={() => openFileDialog(f.key)} className="btn btn-light">Choose Files</button>
                          <span className="text-sm text-gray-600">{files[f.key].length ? `${files[f.key].length} file(s) selected` : "No files selected"}</span>
                        </div>
                        <div className="mt-2 space-y-1">
                          {files[f.key].map((file, i) => (
                            <div key={i} className="flex items-center justify-between gap-2 text-sm">
                              <div className="truncate">{file.name}</div>
                              <button type="button" onClick={() => removeFile(f.key, i)} className="text-red-500 text-xs">Remove</button>
                            </div>
                          ))}
                        </div>
                ...
                {/* SECTION C: Office Use (read-only for now) */}
                {/* <Card title={"SECTION C: (For Office Use)"}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 ">
                    <input placeholder="Jamaat Name" value={form.officeUseInfo.jamaatName} onChange={(e) => setField("officeUseInfo.jamaatName", e.target.value)} className="mt-1 w-full border rounded p-2 border-gray-200 bg-gray-50" readOnly />
                    <input placeholder="Membership Number" value={form.officeUseInfo.membershipNumber} onChange={(e) => setField("officeUseInfo.membershipNumber", e.target.value)} className="mt-1 w-full border rounded p-2 border-gray-200 bg-gray-50" readOnly />
                    <div>
                      ...
                      <Select
                        ...
                        value={
                          form.officeUseInfo.belongsToJamaat
                            ? {
                              value: form.officeUseInfo.belongsToJamaat,
                              label: form.officeUseInfo.belongsToJamaat,
                            }
                            : null
                        }
                        onChange={(selected) =>
                          setField("officeUseInfo.belongsToJamaat", selected?.value || "")
                        }
                        placeholder="Select"
                        isDisabled={true}
                      />
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        value={
                          form.officeUseInfo.supportProvided
                            ? {
                              value: form.officeUseInfo.supportProvided,
                              label: form.officeUseInfo.supportProvided,
                            }
                            : null
                        }
                        onChange={(selected) =>
                          setField("officeUseInfo.supportProvided", selected?.value || "")
                        }
                        placeholder="Select"
                        isDisabled={true}
                      />
                    ...
                    <label className="mt-2 text-sm">Authorized Signature (Office)</label>
                    <input value={form.officeUseInfo.authorizedSignature.name} onChange={(e) => setField("officeUseInfo.authorizedSignature.name", e.target.value)} className="mt-1 w-full border rounded p-2 border-gray-200 bg-gray-50" readOnly />
                    <input value={form.officeUseInfo.authorizedSignature.designation} onChange={(e) => setField("officeUseInfo.authorizedSignature.designation", e.target.value)} className="mt-1 w-full border rounded p-2 border-gray-200 bg-gray-50" readOnly />
                  </div>
                </Card> */}
                <Card title="SECTION C: (For Office Use)">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
                    ...
                    <input
                      ...
                      value={form.officeUseInfo.jamaatName}
                      onChange={(e) => setField("officeUseInfo.jamaatName", e.target.value)}
                      className="w-full border rounded p-2 border-gray-200"
                    />
                    <input
                      placeholder="Membership Number"
                      value={form.officeUseInfo.membershipNumber}
                      onChange={(e) => setField("officeUseInfo.membershipNumber", e.target.value)}
                      className="w-full border rounded p-2 border-gray-200"
                    />
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={form.officeUseInfo.zakatDeserving}
                        onChange={(e) =>
                          setField("officeUseInfo.zakatDeserving", e.target.checked)
                        }
                        className="h-5 w-5"
                      />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setField(
                              "officeUseInfo.authorizedSignature.designation",
                              e.target.value
                            )
                          }
                          className="border rounded p-2 border-gray-200"
                    ...
                    <div className="md:col-span-2">
                      ...
                      <Select
                        ...
                        value={
                          form.officeUseInfo.channelOfSubmission
                            ? {
                              value: form.officeUseInfo.channelOfSubmission,
                              label: form.officeUseInfo.channelOfSubmission,
                            }
                            : null
                        }
                        onChange={(selected) =>
                          setField("officeUseInfo.channelOfSubmission", selected?.value || "")
                        }
                    ...
                    <div className="md:col-span-2 mt-4 border-t pt-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <input
                          ...
                          value={form.officeUseInfo.memfOffice.studentCode}
                          onChange={(e) =>
                            setField("officeUseInfo.memfOffice.studentCode", e.target.value)
                          }
                          className="border rounded p-2 border-gray-200"
                        />
                        <input
                          ...
                          value={form.officeUseInfo.memfOffice.assessmentDate}
                          onChange={(e) =>
                            setField("officeUseInfo.memfOffice.assessmentDate", e.target.value)
                          }
                          className="border rounded p-2 border-gray-200"
                        />
                        <input
                          ...
                          value={form.officeUseInfo.memfOffice.interviewDate}
                          onChange={(e) =>
                            setField("officeUseInfo.memfOffice.interviewDate", e.target.value)
                          }
                          className="border rounded p-2 border-gray-200"
                        />
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            value={
                              form.officeUseInfo.memfOffice.decision
                                ? {
                                  value: form.officeUseInfo.memfOffice.decision,
                                  label: form.officeUseInfo.memfOffice.decision,
                                }
                                : null
                            }
                            onChange={(selected) =>
                              setField("officeUseInfo.memfOffice.decision", selected?.value || "")
                            }
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            value={
                              form.officeUseInfo.memfOffice.category
                                ? {
                                  value: form.officeUseInfo.memfOffice.category,
                                  label: form.officeUseInfo.memfOffice.category,
                                }
                                : null
                            }
                            onChange={(selected) =>
                              setField("officeUseInfo.memfOffice.category", selected?.value || "")
                            }
                        ...
                        <div className="md:col-span-2 mt-2">
                          ...
                          <input
                            placeholder="Granted For (e.g. Monthly Fee, Books)"
                            value={form.officeUseInfo.memfOffice.scholarship.grantedFor}
                            onChange={(e) =>
                              setField(
                                "officeUseInfo.memfOffice.scholarship.grantedFor",
                                e.target.value
                              )
                            }
                          ...
                          <input
                            ...
                            value={form.officeUseInfo.memfOffice.scholarship.totalAmount}
                            onChange={(e) =>
                              setField(
                                "officeUseInfo.memfOffice.scholarship.totalAmount",
                                e.target.value
                              )
                            }
                            className="border rounded p-2 border-gray-200 w-full mt-2"
                          />
                        </div>
                        ...
                        <textarea
                          ...
                          value={form.officeUseInfo.memfOffice.panelComments}
                          onChange={(e) =>
                            setField("officeUseInfo.memfOffice.panelComments", e.target.value)
                          }
                          className="border rounded p-2 border-gray-200 w-full md:col-span-2"
                        />
                        ...
                        <div className="md:col-span-2 mt-2">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <input
                              placeholder="Name"
                              value={form.officeUseInfo.memfOffice.reviewPanelSignature.name}
                              onChange={(e) =>
                                setField(
                                  "officeUseInfo.memfOffice.reviewPanelSignature.name",
                                  e.target.value
                                )
                              }
                              className="border rounded p-2 border-gray-200"
                            ...
                            <input
                              placeholder="Designation"
                              value={
                                form.officeUseInfo.memfOffice.reviewPanelSignature.designation
                              }
                              onChange={(e) =>
                                setField(
                                  "officeUseInfo.memfOffice.reviewPanelSignature.designation",
                                  e.target.value
                                )
                              }
                              className="border rounded p-2 border-gray-200"
                            ...
                            <input
                              placeholder="Stamp (URL / Base64)"
                              value={form.officeUseInfo.memfOffice.reviewPanelSignature.stamp}
                              onChange={(e) =>
                                setField(
                                  "officeUseInfo.memfOffice.reviewPanelSignature.stamp",
                                  e.target.value
                                )
                              }
                              className="border rounded p-2 border-gray-200"
                    ...
                    <div className="md:col-span-2 text-right mt-4">
                      <Button
                        text="Submit Office Use Section"
                        className="btn-dark"
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <Button text={loading ? "Submitting..." : "Submit Application"} isLoading={loading} onClick={handleSubmit} />
                </div>
